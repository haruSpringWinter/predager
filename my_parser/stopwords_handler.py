...
def remove_stopwords(words: list, swlist: list = None) -> list:
    ...
    if swlist == None:
        swlist = stopwords
    words = [word for word in words if word not in swlist]
    return words
...
def get_stop_words(docs: list, n: int = 100, min_freq: int = 1) -> list:
    fdist = Counter()
    for doc in docs:
        for word in doc:
            fdist[word] += 1
...
def init_stop_words(docs: list, n: int = 100, min_freq: int = 1) -> None:
    global stopwords
    stop_words_freq = get_stop_words(docs, n, min_freq)
    stopwords = stopwords + stop_words_freq
...
if __name__ == "__main__":
    ...
    sentences = [
        ...
        "単語のベクトル表現では、文字列である単語をベクトルに変換する処理を行います。なぜ文字列からベクトルに変換するのかとrいうと、文字列は可変長で扱いにくい、類似度の計算がしにくい等の理由が挙げられます。ベクトル表現するのにも様々な手法が存在するのですが、以下の2つについて紹介します。",
    ...
    most_commons = get_stop_words(docs_with_surfaces, 10, 1)
    print("most common terms:")
    for elem in most_commons:
        print(elem)


    text_example = sentences[1]
    nodes = to_morph(text_example)
    surfaces = []
    for node in nodes:
        surfaces.append(node[0])

    removed = remove_stopwords(surfaces, most_commons)
    print(removed)

    print("length of vanila vs removed : " + str(len(nodes)) + " vs "  + str(len(removed)))
